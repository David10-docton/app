    ...
    <div className="min-h-screen bg-rp-bg">
      ...
      <header className="bg-slate-900/80 backdrop-blur-xl border-b border-slate-700/50 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link href="/seller" className="flex items-center gap-2">
            ...
            <div>
              <span className="text-sm font-bold text-white hidden sm:block">Espace Vendeur</span>
              <div className="flex items-center gap-1">
                <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                <span className="text-[10px] text-slate-400">4.8 • Verified</span>
              </div>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navLinks.map(link => (
              <Link key={link.href} href={link.href}
                className="relative px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg transition-colors">
                {link.label}
                {link.badge && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-rp-primary text-white text-[9px] rounded-full flex items-center justify-center font-bold">{link.badge}</span>}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-rp-primary text-white text-[9px] rounded-full flex items-center justify-center font-bold">5</span>
            </div>
            <button onClick={() => setMobileMenu(!mobileMenu)} className="lg:hidden p-2 text-slate-400 hover:text-white">
              {mobileMenu ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
            </button>
            <button onClick={logout} className="hidden sm:flex p-2 text-slate-400 hover:text-red-400"><LogOut className="w-4 h-4" /></button>
          </div>
        </div>
          ...
          <div className="lg:hidden border-t border-slate-700/50 px-4 py-3 space-y-1 slide-up">
            {navLinks.map(link => (
              <Link key={link.href} href={link.href} onClick={() => setMobileMenu(false)}
                className="block px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg">
      ...
      <div className="bg-gradient-to-r from-blue-600/20 to-blue-500/10 border-b border-blue-500/20">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-extrabold text-white">{todayStats.todayRevenue.toLocaleString()} <span className="text-sm font-medium text-blue-300">FCFA</span></p>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6 pb-24 lg:pb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Nvelles demandes', value: todayStats.newRequests, color: 'text-rp-primary' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white">Nouvelles demandes</h2>
                <span className="w-5 h-5 bg-rp-primary text-white text-[10px] rounded-full flex items-center justify-center font-bold">5</span>
              </div>
              <Link href="/seller/requests" className="text-xs text-rp-primary font-medium flex items-center gap-1 hover:underline">Voir tout <ChevronRight className="w-3 h-3" /></Link>
            </div>
            <div className="space-y-2">
              {recentRequests.map(req => (
                <Link key={req.id} href={`/seller/requests/${req.id}`}
                  className="block bg-slate-800/50 border border-slate-700/50 rounded-xl p-3 hover:border-slate-600 transition-colors card-hover">
                  <div className="flex items-start gap-3">
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${req.urgent ? 'bg-red-500/10' : 'bg-slate-700/50'}`}>
                      {req.urgent ? <AlertCircle className="w-4 h-4 text-rp-primary" /> : <Package className="w-4 h-4 text-slate-400" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="text-xs font-semibold text-white truncate">{req.part}</h3>
                        {req.urgent && <span className="text-[8px] bg-rp-primary text-white px-1.5 py-0.5 rounded-full flex-shrink-0">URGENT</span>}
                      </div>
                      <p className="text-[10px] text-slate-400">{req.buyer} • {req.type}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] text-slate-500 flex items-center gap-0.5"><MapPin className="w-2.5 h-2.5" /> {req.location}</span>
                        <span className="text-[9px] text-slate-500">{req.time}</span>
                        <span className="text-[9px] text-rp-primary font-medium">{req.responses} offres</span>
                      </div>
                    </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-bold text-white">Ventes récentes</h2>
              <Link href="/seller/orders" className="text-xs text-rp-primary font-medium flex items-center gap-1 hover:underline">Voir tout <ChevronRight className="w-3 h-3" /></Link>
            </div>
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden">
              {recentSales.map((sale, i) => (
                <div key={sale.id} className={`p-3 flex items-center gap-3 ${i < recentSales.length - 1 ? 'border-b border-slate-700/50' : ''}`}>
                  <div className="w-9 h-9 bg-emerald-500/10 rounded-lg flex items-center justify-center flex-shrink-0">
                    <TrendingUp className="w-4 h-4 text-emerald-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-white truncate">{sale.part}</p>
                    <p className="text-[10px] text-slate-400">{sale.buyer} • {sale.time}</p>
                  </div>
        ...
        <section className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5">
          <h3 className="text-sm font-bold text-white mb-4">📊 Performance du mois</h3>
            ...
            {[
              ...
              <div key={kpi.label}>
                ...
                <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
                  <div className="h-full bg-rp-primary rounded-full transition-all" style={{ width: `${Math.min(100, kpi.pct)}%` }} />
                </div>
              </div>
            ))}
          </div>
